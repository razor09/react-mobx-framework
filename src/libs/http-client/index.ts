...
export class HttpClient {
  ...
  private generateUrl(endpoint: string, queryParams?: QueryParams): string {
    ...
    if (queryParams) {
      const result = Helpers.shake(queryParams, invalidRequestParams)
  ...
  public interpolate(value: string, ...args: StringOrNumber[]): string {
    const segments = value.split('/')
    let index = 0
    for (let i = 0; i < segments.length; i++) {
      if (segments[i].startsWith(':') && args[index]) {
        segments[i] = args[index].toString()
        index++
      }
    }
  ...
  public async post<R>(endpoint: string, body?: PlainObjectOrArray): Promise<R> {
    try {
      return await this.call({
        method: 'post',
        endpoint,
  ...
  public async put<R>(endpoint: string, body?: PlainObjectOrArray): Promise<R> {
    try {
      return await this.call({
        method: 'put',
        endpoint,
  ...
  public async delete<R>(endpoint: string, body?: PlainObjectOrArray): Promise<R> {
    try {
      return await this.call({
        method: 'delete',
        endpoint,
  ...
  public async loadStream(endpoint: string, queryParams?: QueryParams, onReadStream?: OnReadStream): Promise<Uint8Array[]> {
    try {
      ...
      if (response.body) {
        const total = Helpers.toInteger(response.headers.get('content-length'))
        if (total) {
